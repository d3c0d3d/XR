...
namespace XR.Core.OptionCommand
{
    public sealed class OptionSet : KeyedCollection<string, Option>
    {
        ...
        public List<string> Parse(IEnumerable<string> arguments, bool introduceTrace = true)
        {
            ...
            foreach (var argument in arguments)
            {
                ...
                if (introduceTrace && onlyFirst)
                {
                    _introduceTrace = true;
                    arg = $"-{argument}";
                    onlyFirst = false;
                }
                ...
                if (!process)
                {
                    Unprocessed(unprocessed, def, c, arg);
                    continue;
                }
                if (!Parse(arg, c))
                    Unprocessed(unprocessed, def, c, arg);
        ...
        public void WriteOptionDescriptions(TextWriter o, params string[] notshow)
        {
            foreach (var p in this)
            {
                if (notshow.Any(x => x == p.Prototype))
                    continue;
        ...
        private bool WriteOptionPrototype(TextWriter o, Option p, ref int written)
        {
            ...
            if (names[i].Length == 1)
            {
                Write(o, ref written, "  " + (!_introduceTrace ? "-" : ""));
